...
export default function GiocatoreEstratto(props) {
    ...
    return (<div className={`${props.style["singolo-estratto"]} ${assegnato ? props.style["gia-assegnato"] : ""}`}>
        <p style={{ maxWidth: '100%', overflowX: 'auto' }} className={props.style["p-lista-intera"]}>
            ...
            {!assegnato && props.role === 'admin' && (<button className={props.style["btn-assegna-svincolato"]} disabled={props.isDoingRequest} onClick={() => props.assegnaGiocatore(allenatoreScelto, props.giocatore.Id, puntata, props.calcolaTotaleSpeso(allenatoreScelto))}>Assegna</button>)}
        </p>
    </div>)
}